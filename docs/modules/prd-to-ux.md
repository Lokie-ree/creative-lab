---
name: prd-to-ux
description: Translate PRDs for interactive educational modules into UX specifications through 6 foundational passes. Use before generating build-order prompts or implementation.
---
...
**Context:** This is specifically for interactive learning modules using React Three Fiber, stage-based flows, and the Observe → Manipulate → Discover → Celebrate pattern.
...
Pattern: `{prd-basename}-ux-spec.md` (or just `ux-spec.md` if PRD is `prd.md`)
...
4. Map to stage-based visibility (what's visible in Observe vs Manipulate vs Discover vs Celebrate)
